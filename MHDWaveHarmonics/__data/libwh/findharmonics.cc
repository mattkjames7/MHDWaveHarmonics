...
void FindHarmonics(	float *B, float *R, float *s, float *halpha, 
	...
	/* shoot waves */
	ShootWaves(n,Va,dlndx,s,df,x0,nh,HarmInds,nIter,freqs);
...
void FindHarmonicsComplex(	float *B, float *R, float *s, float *halpha, 
	 ...
	 * 
	 */
	float Va[n-1], dlndx[n-1], HarmPhase[nh];
	int i;
	...
	/* shoot waves */
	ShootWavesComplex(n,Va,dlndx,s,nh,HarmPhase,HarmInds,x0,Success,nIter,freqs);
...
void FindHarmonicsPMD(	float *B, float *s, float *halpha, float *pmd,
	...
	/* populate Va array (km/s) */
	CalcFieldLineVaMidPMD(B,pmd,s,RhoBG,n,Va);
...
void ShootWaves(	int n, float *Va, float *dlndx, float *s, 
	...
	float yarr[nmax], farr[nmax], fc = df, y[n], f0 = x0;
...
void ShootWavesComplex(	int n, float *Va, float *dlndx, float *s,
	...
	float yr[n], yi[n], phase[n], CurrTargetPhase;
	int i;
	...
	for (i=0;i<nh;i++) {
		...
		CurrTargetPhase = HarmPhase[i];
		Success[i] = neldermead(&x0[i],1,DiffLambda,2000,1e-6,1e-6,true,&nIter[i],&freqs[i]);
		
		SolveWaveComplex(freqs[i],s,n,Va,dlndx,yr,yi,phase,&mxr,&mxi);
